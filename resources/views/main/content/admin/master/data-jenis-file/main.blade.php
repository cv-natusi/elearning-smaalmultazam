...
<script>
	var routeDatatable = "{{route('admin.jenisFile.main')}}";
	var routeJenisFileAdd = "{{route('admin.jenisFile.add')}}";
	var routeJenisFileDelete = "{{route('admin.jenisFile.delete')}}";
	...
	async function dataTable() {
		...
		const templateTambah = `
			<button onclick="tambahJenisFile()" class='btn btn-primary p-2 w-100'><i class='bx bx-plus' ></i>Tambah</button>
		`;

		$("div.templateTambah").html(templateTambah)
	}

	function tambahJenisFile(id='') {
		$('.main-page').hide();
		var url = routeJenisFileAdd
		$.post(url, {id:id})
		.done(function(data){
